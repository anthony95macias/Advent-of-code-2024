...
Deno.test("Counts occurrences of 'XMAS' with real data", async () => {
  ...
  // Assertions
  const expectedOccurrences = 2618;
  const expectedCowsayMessage = cowsay.say({
    text: `The word '${word}' occurs ${expectedOccurrences} times.`,
  });

  assertEquals(xmases, expectedOccurrences, "The count of 'XMAS' is incorrect.");
  assertEquals(answer, expectedCowsayMessage, "The cowsay output is incorrect.");
});
